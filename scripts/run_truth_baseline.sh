...
if [[ "$vnc_mode" == "1" ]]; then
  ...
  if [[ "${MOVEIT_USE_VIRTUALGL:-1}" == "1" ]]; then
    ...
    echo "[阶段 2] 启用 VirtualGL：A800 EGL 设备 ${MOVEIT_VGL_DEVICE:-egl0}。"
    exec vglrun -d "${MOVEIT_VGL_DEVICE:-egl0}" -- "${root_dir}/.native_env/bin/python" -m vision_moveit_demo.truth_baseline "$@"
  fi
else
  export MUJOCO_GL="${MUJOCO_GL:-egl}"
fi

exec "${root_dir}/.native_env/bin/python" -m vision_moveit_demo.truth_baseline "$@"
